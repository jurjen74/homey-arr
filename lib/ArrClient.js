...
class ArrClient {
  ...
  getRecords(path, query, mapFn) {
    ...
    return this._fetchText(fullUrl).then((text) => {
      ...
      const records = [];
      if (markerIdx !== -1) {
        for (const itemStr of iterJsonArray(text.slice(markerIdx + marker.length - 1))) {
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            if (kb > 50) {
              ...
              const heapUsedMB  = Math.round(heapUsed  / 1048576);
              const heapTotalMB = Math.round(heapTotal / 1048576);
              const sysFreeMB   = Math.round(os.freemem()  / 1048576);
              const sysTotalMB  = Math.round(os.totalmem() / 1048576);
              console.log(`[ArrClient] ${this.appName} ${parsed.pathname} — ${kb} KB, heap ${heapUsedMB}/${heapTotalMB} MB, sys ${sysFreeMB}/${sysTotalMB} MB free`);
            }
