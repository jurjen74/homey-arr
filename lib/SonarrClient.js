...
class SonarrClient extends ArrClient {
  ...
  getRecentHistorySlim(pageSize = 15) {
    return this.getRecords('/api/v3/history', {
      ...
      // /api/v3/history embeds the full episode object in every record regardless of flags (see
      // CLAUDE.md), so these four scalars are already parsed — keeping them costs nothing beyond
      // the copy, and they let the *_downloaded triggers report the real title, numbering and age
      // instead of guessing from the release name.
      episode: {
