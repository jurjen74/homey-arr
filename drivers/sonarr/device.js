...
// An upgrade writes the file-deletion when Sonarr commits to replacing, and the import lands
// once the file is in place — adjacent by id, but separated in time. Measured on a live
// instance: 1-8 s, so only ~13% straddle a 60 s poll and need the hint carried across polls.
// Ten minutes is 75x that maximum, which covers a slow import without keeping a hint from an
// import that never lands (failed, out of disk) around long enough to mislabel the next genuine
// first import of the same episode.
// Radarr's gap is two orders of magnitude larger and its TTL is sized separately — see
// drivers/radarr/device.js. Do not copy a value between the two.
...
class SonarrDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    // episodeId → timestamp of an 'Upgrade' file deletion, used to mark the import that
  ...
  async _updateHistory() {
    ...
    const records = await collectNewHistory({
      fetchPage: (page) => this._client.getRecentHistorySlim(HISTORY_PAGE_SIZE, page),
    ...
    for (const record of records) {
      ...
      // first import of an episode *and* a later upgrade of it, the first import consumes the
      ...
      if (isImport) {
        ...
        const isUpgrade = this._upgradeHints.delete(record.episodeId);
  ...
  // A quality upgrade is not marked on the import itself — isUpgrade exists only in Sonarr's
  // webhook payload. The history-API signal is this separate deletion record, written when
  // Sonarr commits to replacing the file, which the following import then consumes.
  _registerUpgradeHint(record, nowMs) {
    if (record.eventType === 'episodeFileDeleted' && record.data?.reason === 'Upgrade' && record.episodeId) {
      this._upgradeHints.set(record.episodeId, nowMs);
    }
  }
